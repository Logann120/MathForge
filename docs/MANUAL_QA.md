...
| --- | --- | --- |
| Expanded-difficulty Topic mode worksheet | Topic mode, each supported topic, `Worksheet only`, each of `Easy`, `Medium`, and `Hard` | Difficulty selector shows exactly `Easy`, `Medium`, and `Hard`; each difficulty generates without a stack trace; worksheet, solution key, exports tab, individual downloads, ZIP bundle, Canvas CSV, and LibGuides-safe HTML controls still appear; generated-output summary shows the selected difficulty. |
| Expanded-difficulty Topic mode resource pack | Topic mode, each supported topic, `Full Resource Pack`, each of `Easy`, `Medium`, and `Hard` | Difficulty selector shows exactly `Easy`, `Medium`, and `Hard`; each difficulty generates a full resource pack; study guide, common mistakes, tutor notes, practice quiz, export buttons, and generated-output summary remain present; summary reflects the selected difficulty. |
| Expanded-difficulty Learning Objective mode | Learning Objective mode, each College Algebra module/objective, worksheet or resource pack | Difficulty selector shows exactly `Easy`, `Medium`, and `Hard`; generated-output summary identifies Learning Objective mode, mapped topic, and the selected difficulty. |
...
- Confirm every supported topic exposes `Easy`, `Medium`, and `Hard`.
- Generate each supported topic as a worksheet and resource pack at `Easy`, `Medium`, and `Hard`.
...
- Confirm every College Algebra objective exposes `Easy`, `Medium`, and `Hard`.
- Confirm generated-output summaries show the selected difficulty.
...
- [ ] Topic mode exposes Easy, Medium, and Hard for every supported topic.
- [ ] Every supported topic generates worksheet-only and full resource-pack output at Easy, Medium, and Hard.
